...
pub async fn add_task(pool: &Pool, user: DbUser, magnet: ShortMagnet, directory: &Uuid) -> Result<DownloadTask, DbError> {
    let connection = pool.get().await?;
    let query = "INSERT INTO tasks(id, user_id, magnet, directory, status) VALUES($1,$2,$3,$4,$5);";
    let task_id = Uuid::new_v4();
    connection.query(query, &[&task_id.to_string(), &user.id, &String::from(magnet), &directory.to_string(), &TaskStatus::Created]).await?;
    get_task_by_id(pool, &task_id).await
}

pub(crate) async fn get_task_by_id(pool: &Pool, id: &Uuid) -> Result<DownloadTask, DbError> {
    let connection = pool.get().await?;
    let query = "SELECT id, user_id, magnet, directory, status,description FROM tasks WHERE id=$1;";
    let rows = connection.query(query, &[&id.to_string()]).await?;
    let row = rows.get(0).unwrap();
